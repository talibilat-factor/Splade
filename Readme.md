...
# --- Alternatively: single script helper ---
# You can perform the environment setup, (optional) data download, and either toy or full run
# with the convenience script added in `scripts/readme_workflow.sh`:
#
#   chmod +x scripts/readme_workflow.sh
#   ./scripts/readme_workflow.sh --setup-env --toy-run     # toy example
#   ./scripts/readme_workflow.sh --full-run                # full example (will download data if missing)
#   ./scripts/readme_workflow.sh --help                    # list options
#
# The script is idempotent and accepts flags such as `--download-data-only` or `--no-conda`.
```